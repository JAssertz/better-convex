...
import { AuthFunctions, Triggers } from './client';
    ...
    adapter: ({ options }) => {
      ...
      return {
        ...
        create: async ({ model, data, select }): Promise<any> => {
          ...
          const onCreateHandle =
            config.authFunctions?.onCreate && config.triggers?.[model]?.onCreate
              ...
              : undefined;
          return ctx.runMutation(internal.betterAuth.adapter.create, {
            input: { model, data },
            select,
        ...
        findOne: async (data): Promise<any> => {
          if (data.where?.every((w) => w.connector === 'OR')) {
            for (const w of data.where) {
              const result = await ctx.runQuery(
                internal.betterAuth.adapter.findOne,
                {
                  ...data,
                  where: parseWhere([w]),
                }
          ...
          return await ctx.runQuery(internal.betterAuth.adapter.findOne, {
            ...data,
        ...
        findMany: async (data): Promise<any[]> => {
          ...
          const result = await handlePagination(
            async ({ paginationOpts }) => {
              return await ctx.runQuery(internal.betterAuth.adapter.findMany, {
                ...data,
                where: parseWhere(data.where),
                paginationOpts,
              });
          ...
          if (data.where?.length === 1 && data.where[0].operator === 'eq') {
            const onUpdateHandle =
              ...
              config.triggers?.[data.model]?.onUpdate
                ...
                : undefined;
            return ctx.runMutation(internal.betterAuth.adapter.updateOne, {
              input: {
        ...
        delete: async (data) => {
          ...
          const onDeleteHandle =
            ...
            config.triggers?.[data.model]?.onDelete
              ...
              : undefined;
          await ctx.runMutation(internal.betterAuth.adapter.deleteOne, {
            input: {
              model: data.model,
              where: parseWhere(data.where),
            },
        ...
        updateMany: async (data) => {
          ...
          const result = await handlePagination(async ({ paginationOpts }) => {
            return await ctx.runMutation(
              internal.betterAuth.adapter.updateMany,
              {
                input: {
                  ...(data as any),
                  where: parseWhere(data.where),
