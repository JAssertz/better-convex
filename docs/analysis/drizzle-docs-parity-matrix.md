...
| `column-types/cockroach` | N/A (SQL-only) | `www/content/docs/orm/column-types.mdx` | Better-Convex documents a Convex subset; SQL dialect-specific types are not applicable. |
| `column-types/mssql` | N/A (SQL-only) | `www/content/docs/orm/column-types.mdx` | Better-Convex documents a Convex subset; SQL dialect-specific types are not applicable. |
| `column-types/mysql` | N/A (SQL-only) | `www/content/docs/orm/column-types.mdx` | Better-Convex documents a Convex subset; SQL dialect-specific types are not applicable. |
| `column-types/pg` | PARTIALLY ADAPTED | `www/content/docs/orm/column-types.mdx` | Only Better-Convex builders are documented (text/integer/number/boolean/bigint/id/vector). |
| `column-types/singlestore` | N/A (SQL-only) | `www/content/docs/orm/column-types.mdx` | Better-Convex documents a Convex subset; SQL dialect-specific types are not applicable. |
| `column-types/sqlite` | N/A (SQL-only) | `www/content/docs/orm/column-types.mdx` | Better-Convex documents a Convex subset; SQL dialect-specific types are not applicable. |
...
| `data-querying` | PARTIALLY ADAPTED | `www/content/docs/orm/queries.mdx`<br/>`www/content/docs/orm/limitations.mdx` | Better-Convex only covers relational-style querying; SQL-like query builder is N/A. |
| `delete` | PARTIALLY ADAPTED | `www/content/docs/orm/delete.mdx`<br/>`www/content/docs/orm/mutations.mdx` | No SQL `WITH` clauses, limit, or orderBy on deletes; runtime enforcement (RLS/constraints). |
...
| `indexes-constraints` | PARTIALLY ADAPTED | `www/content/docs/orm/indexes-constraints.mdx` | Constraints are runtime-enforced (ORM only); SQL-only constraints are N/A. |
| `insert` | PARTIALLY ADAPTED | `www/content/docs/orm/insert.mdx`<br/>`www/content/docs/orm/mutations.mdx` | SQL output sections omitted; Convex inserts return `void` unless `returning()` is used. |
...
| `operators` | PARTIALLY ADAPTED | `www/content/docs/orm/operators.mdx` | Better-Convex supports RQBv2 object filters + mutation filter helpers; SQL-only operators (including `exists`/`notExists`) are N/A. |
...
| `relations-schema-declaration` | PARTIALLY ADAPTED | `www/content/docs/orm/relations.mdx` | Soft relations + `defineRelations` mirrored; SQL FK declaration is different in Convex. |
| `relations-v1-v2` | N/A (SQL-only) |  |  |
| `relations-v2` | PARTIALLY ADAPTED | `www/content/docs/orm/relations.mdx` | Mirrors `defineRelations({ tables }, (r) => ...)` and `one/many` config; Convex uses `_id`. |
| `rls` | PARTIALLY ADAPTED | `www/content/docs/orm/rls.mdx` | RLS is enforced at runtime by the ORM (not in the database). |
| `rqb` | N/A (SQL-only) |  | Old Drizzle RQBv1 docs are not mirrored; Better-Convex targets v1 RC-style APIs. |
| `rqb-v2` | PARTIALLY ADAPTED | `www/content/docs/orm/queries.mdx`<br/>`www/content/docs/orm/limitations.mdx` | `extras`, `RAW`, SQL placeholders, prepared statements, and subquery operators (`exists`/`notExists`) are SQL-only and unsupported at runtime. |
...
| `sql-schema-declaration` | PARTIALLY ADAPTED | `www/content/docs/orm/schema.mdx`<br/>`www/content/docs/orm/column-types.mdx`<br/>`www/content/docs/orm/indexes-constraints.mdx` | SQL DDL + migration/tooling sections are omitted; Convex document model differs. |
...
| `update` | PARTIALLY ADAPTED | `www/content/docs/orm/update.mdx`<br/>`www/content/docs/orm/mutations.mdx` | No SQL `UPDATE ... FROM`, limit, or orderBy on mutations; runtime enforcement (RLS/constraints). |
