...
function splitRange(
  ...
  while (
    workingStartBound.length > 0 &&
    workingEndBound.length > 0 &&
    compareValues(workingStartBound[0]!, workingEndBound[0]!) === 0
  ) {
  ...
  const makeCompare = (
    ...
    for (; i < key.length - 1; i++) {
      range.push(['eq', workingIndexFields[i]!, key[i]!]);
    }
    if (i < key.length) {
      range.push([boundType, workingIndexFields[i]!, key[i]!]);
    }
    return range;
  };
  // Stage 1.
  ...
  if (workingEndBound.length === 0) {
    middleRange = makeCompare(workingStartBoundType, workingStartBound);
  } else if (workingStartBound.length === 0) {
    middleRange = makeCompare(workingEndBoundType, workingEndBound);
  } else {
    ...
    middleRange.push([workingEndBoundType, workingIndexFields[0]!, endValue]);
  }
...
function rangeToQuery(range: Bound[]) {
  return (q: any) => {
    ...
    for (const [boundType, field, value] of range) {
      query = query[boundType](field, value);
    }
    return query;
  };
}
...
class ReflectIndexRange {
  #hasSuffix = false;
  lowerBoundIndexKey: IndexKey | undefined = undefined;
  lowerBoundInclusive = true;
  upperBoundIndexKey: IndexKey | undefined = undefined;
