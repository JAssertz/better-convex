...
describe('stream compatibility', () => {
  ...
  test('query round trip with pagination', async () => {
    ...
    await t.run(async (ctx) => {
      ...
      const query = stream(ctx.db, schema)
        .query('foo')
        .withIndex('abc', (q) => q.eq('a', 1).gt('b', 2));
      const resultPage1 = await query.paginate({ limit: 2, cursor: null });
  ...
  test('continueCursor respects stream order', async () => {
    ...
    await t.run(async (ctx) => {
      ...
      const query = stream(ctx.db, schema)
        ...
        .withIndex('abc')
        .order('desc');
      const result = await query.paginate({ limit: 1, cursor: null });
  ...
  test('merge streams', async () => {
    ...
    await t.run(async (ctx) => {
      ...
      const page1 = await fullQuery.paginate({ limit: 2, cursor: null });
  ...
  test('map and flatMap streams', async () => {
    const t = convexTest(schema);
    await t.run(async (ctx) => {
      await ctx.db.insert('foo', { a: 1, b: 2, c: 3 });
      await ctx.db.insert('foo', { a: 1, b: 3, c: 4 });
      ...
      expect(await mapped.collect()).toEqual([
        'doc with c: 3',
        'doc with c: 4',
      ]);
  ...
  test('paginate with 0 limit', async () => {
    const t = convexTest(schema);
    await t.run(async (ctx) => {
      const query = stream(ctx.db, schema)
        .query('foo')
        .withIndex('abc', (q) => q.eq('a', 1));
      const page = await query.paginate({ limit: 0, cursor: '' });
  ...
  test.each([
    { order: 'asc' as const },
    { order: 'desc' as const },
  ])('distinct pagination works', async ({ order }) => {
    const t = convexTest(schema);
    await t.run(async (ctx) => {
      const values = [
        { a: 1, b: 2, c: 3 },
      ...
      const doPaginate = (cursor: string | null) =>
        stream(ctx.db, schema)
          .query('foo')
          .withIndex('abc', (q) => q.eq('a', 1))
          .order(order)
      ...
      const expected = order === 'asc' ? expectedAsc : expectedDesc;
  ...
  test('undefined cursor serialization roundtrips', async () => {
    const schemaWithOptional = defineSchema({
      foo: defineTable({
        a: v.optional(v.number()),
        b: v.number(),
      }).index('ab', ['a', 'b']),
    });
    const t = convexTest(schemaWithOptional);
    await t.run(async (ctx) => {
      ...
      const query = stream(ctx.db, schemaWithOptional)
        .query('foo')
        .withIndex('ab');
      const result = await query.paginate({ limit: 1, cursor: null });
  ...
  test('literal undefined string works', async () => {
    const schemaWithOptionalString = defineSchema({
      foo: defineTable({
        a: v.optional(v.string()),
        b: v.number(),
      }).index('ab', ['a', 'b']),
    });
    const t = convexTest(schemaWithOptionalString);
    await t.run(async (ctx) => {
      await ctx.db.insert('foo', { a: undefined, b: 1 });
      await ctx.db.insert('foo', { a: 'undefined', b: 2 });
      const query = stream(ctx.db, schemaWithOptionalString)
        .query('foo')
        .withIndex('ab');
      const result = await query.paginate({ limit: 1, cursor: null });
      expect(result.continueCursor).toMatch('["undefined",');
