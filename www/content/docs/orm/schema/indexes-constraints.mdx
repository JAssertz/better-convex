...
<Callout icon={<AlertTriangle />}>
Do not create indexes on `createdAt`. In Better Convex ORM, `createdAt` maps to internal `_creationTime`, and explicit `createdAt` indexes can fail. Use `updatedAt` (or another explicit sortable field) for custom indexes.
</Callout>
