...
import { convexTable, defineSchema, id, index, integer, json, text } from 'better-convex/orm';
...
import { convexTable, id, index, integer, text } from 'better-convex/orm';
...
export const update = authMutation
  ...
  .mutation(async ({ ctx, input }) => {
    ...
    await ctx.orm
      .update(organization)
      .set(input)
      .where(eq(organization.id, ctx.user.activeOrganization!.id));
...
export const listUserInvitations = authQuery
  ...
  .query(async ({ ctx }) => {
    ...
    return asyncMap(invitations, async (inv) => {
      ...
      const inviter = await ctx.orm.query.user.findFirst({
...
export const listPendingInvitations = authQuery
  ...
  .query(async ({ ctx, input }) => {
    ...
    if (!org || ctx.user.activeOrganization?.id !== org.id) return [];
