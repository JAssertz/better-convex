...
**Already in production?** Use the [migration component](https://github.com/get-convex/better-auth) to copy data from component tables to your app tables before deleting the `betterAuth` folder.
</Callout>
...
If you're using Next.js, migrate the server-side helpers too.

### Server Setup

**Before:**
```ts showLineNumbers title="lib/auth-server.ts" {2,4-11}
...
**Before:**
```tsx showLineNumbers title="app/layout.tsx" {3,7}
import "./globals.css";
...
For TanStack Start, the migration is similar to the core steps above. The `@convex-dev/better-auth` package provides `convexBetterAuthReactStart` which better-convex doesn't replace — continue using it from the original package.

### Server Setup

Keep using `@convex-dev/better-auth` for TanStack Start server helpers:
...
Update the client to use `createAuthMutations`:

```ts showLineNumbers title="src/lib/auth-client.tsx" {3,12-17}
import { convexClient } from '@convex-dev/better-auth/client/plugins';
import { createAuthClient } from 'better-auth/react';
import { createAuthMutations } from 'better-convex/react';
...
Use TanStack Router's `beforeLoad` for auth guards:
