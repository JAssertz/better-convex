...
```ts title="convex/lib/triggers.ts" showLineNumbers {1-2,6-25}
...
triggers.register('user', {
  async afterCreate(ctx, _user) {
    const row = await ctx.orm.query.stats.findFirst({
      orderBy: { _creationTime: 'desc' },
    });
    if (!row) return;
    await ctx.orm
      .update(stats)
      .set({ totalUsers: row.totalUsers + 1 })
      .where(eq(stats._id, row._id));
  },
  async afterDelete(ctx, _user) {
    const row = await ctx.orm.query.stats.findFirst({
      orderBy: { _creationTime: 'desc' },
    });
    if (!row) return;
