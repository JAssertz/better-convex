...
Payment and subscription system with [Better Auth's Polar plugin](https://polar.sh/docs/integrate/sdk/adapters/better-auth). Provides checkout flows, subscription management, usage-based billing, and customer portals.

**Architecture:** Better Auth Polar plugin → Webhooks → Convex mutations → Database
...
```ts title="convex/lib/polar-polyfills.ts"
...
```ts title="convex/lib/polar-client.ts"
...
```ts title="convex/lib/polar-helpers.ts"
...
```tsx title="src/components/checkout-button.tsx"
...
```ts title="convex/functions/polarCustomer.ts"
import '../lib/polar-polyfills';
...
```ts title="convex/functions/polarSubscription.ts"
import '../lib/polar-polyfills';
