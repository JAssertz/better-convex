...
If you use cRPC, you can enable Date hydration for system timestamps with `createOrm({ types: { date: true } })`. In that mode, ORM rows expose `createdAt: Date`.
...
See [Column Types](/docs/db/orm/column-types) for the full ORM builder reference and Drizzle mapping notes.
...
<Cards>
  <Card title="Column Types" href="/docs/db/orm/schema/column-types">
