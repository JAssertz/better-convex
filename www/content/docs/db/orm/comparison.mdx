...
<Tabs groupId="migration" items={['Drizzle', 'Better-Convex']} persist>
  ...
  <Tab value="Better-Convex">
    ...
    const requiredUser = await db.query.users.findFirstOrThrow({
      where: { _id: userId },
    });
...
| n/a | `db.query.*.findFirstOrThrow()` | Better-Convex sugar; throws if no row exists |
