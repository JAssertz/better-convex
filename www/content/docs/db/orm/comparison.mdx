...
<Tabs groupId="migration" items={["Drizzle", "Better-Convex"]} persist>
  ...
  <Tab value="Better-Convex">
    ...
    const users = convexTable('users', {
      ...
      isAdmin: v.optional(v.boolean()), // No .default() yet
    });
...
|-------------|-------------------|---------------|
| `pgTable()` / `mysqlTable()` | `convexTable()` | ✅ API identical |
| `relations()` | `relations()` | ✅ 100% identical |
| `db.query.*.findMany()` | `db(ctx).query.*.findMany()` | ✅ Add ctx |
| `db.query.*.findFirst()` | `db(ctx).query.*.findFirst()` | ✅ Add ctx |
| `db.insert().values()` | `db(ctx).insert.*.values()` | ✅ Add ctx |
| `db.update().set().where()` | `db(ctx).update.*.set().where()` | ✅ Add ctx |
| `db.delete().where()` | `db(ctx).delete.*.where()` | ✅ Add ctx |
| `eq()`, `ne()`, `gt()`, etc. | Same | ✅ 100% identical |
| `and()`, `or()`, `not()` | Same | ✅ 100% identical |
| `with` (relations) | `with` | ✅ 100% identical |
| `where` (filtering) | `where` | ✅ 100% identical |
| `limit` / `offset` | `limit` / `offset` | ✅ 100% identical |
| `orderBy()` | `orderBy()` | ✅ See docs |
| `.select()` (columns) | ❌ Not supported | ⚠️ Use post-filter |
| `like()` / `ilike()` | ❌ Not supported | ⚠️ Use search or post-filter |
| Raw SQL | ❌ Not supported | ❌ Convex uses documents |
| Migrations | ❌ Not supported | ❌ Convex is schemaless |
...
### Category 2: ⚠️ Limited/Workaround
...
| Subqueries | Supported | ❌ Not supported (denormalize data) |
...
// ✅ Better-Convex workaround (post-filter)
const users = await db(ctx).query.users.findMany();
const filtered = users.filter(u =>
  u.name.toLowerCase().includes('alice')
);

// ✅ Better approach (use Convex search index)
// Define in schema:
...
|-----------|----------------------|---------------|--------|
| Simple query | ~10ms | ~5ms | 🚀 Better-Convex |
| Query with relations | ~50ms (JOINs) | ~15ms (edges) | 🚀 Better-Convex |
| Real-time updates | Manual polling | Automatic | 🚀 Better-Convex |
| Deep pagination | O(n+offset) | O(log n) | 🚀 Better-Convex |
| Column selection | ✅ Supported | ⚠️ Post-filter | Drizzle |
| Full-text search | ✅ Built-in | ✅ Search indexes | Tie |
