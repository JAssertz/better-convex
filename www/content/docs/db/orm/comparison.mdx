...
- No ent‑object API (`getX()`, `patch()`, lazy promises); use `ctx.table.query.*` and mutation builders
- No edge methods on loaded rows; use relation loading + mutation builders

See [Migrate from Convex Ents](/docs/db/orm/migrate-from-ents) for a step‑by‑step mapping.
...
- [ ] Attach ORM to `ctx.table` in context (create once)
...
| `db.query.*.findMany()` | `db.query.*.findMany()` | Same surface via `ctx.table` |
| `db.query.*.findFirst()` | `db.query.*.findFirst()` | Same surface via `ctx.table` |
...
| Gotcha | Fix |
|---|---|
| **`ctx.table` is missing** | Attach ORM once in context (`createDatabase(...)`) |
