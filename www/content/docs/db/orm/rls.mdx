...
Pass the request context into `createDatabase` so policy functions can access it:
...
`createDatabase` exposes an explicit escape hatch for trusted access:

```ts
const db = createDatabase(ctx.db, ormSchema, ormEdges, { rls: { ctx } });

await db.skipRules.table.query.secrets.findMany();
```

If you attach `db` to your context, you can also surface it as
`ctx.skipRules.table`.
...
- `ctx.db` bypasses RLS; only ORM access (`createDatabase`) enforces rules
