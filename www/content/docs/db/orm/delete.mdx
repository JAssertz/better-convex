...
Let's start with a mutation that deletes a user by `_id`:
...
**Important:** `delete()` without `.where(...)` throws unless you call `.allowFullScan()`. See [Querying Data](/docs/db/orm/queries) for details on `allowFullScan`.
</Callout>
...
```ts showLineNumbers
await ctx.orm.delete(users).where(eq(users._id, userId)).hard();
```
