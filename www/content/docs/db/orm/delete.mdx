...
- Explicit async APIs (`execute({ mode: 'async' })` / `executeAsync()`) cannot be combined with `.paginate()` or `.scheduled(...)`.
...
- Cascade and other FK actions use the same async batching strategy (first page inline, continuation scheduled) when async mode is active.
