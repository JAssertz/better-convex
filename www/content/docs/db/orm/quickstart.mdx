...
## Step 4: Attach ORM to Context (once)

Create a small helper that attaches Better‑Convex ORM to `ctx.table`. Do this once in your context setup.
...
If you use better‑convex cRPC, wire it into the context:
...
- Attached the ORM once as `ctx.table`
...
| Issue | Fix |
|---|---|
| `ctx.table` is missing | Ensure `withOrm(ctx)` is applied in your context |
