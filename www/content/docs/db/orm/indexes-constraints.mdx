...
Here's how to reference another table's `_id`:
...
For referencing non-`_id` columns, use the `foreignKey()` builder:
...
**Tip:** For non-`_id` foreign keys, add an index on the referenced columns (parent table) so inserts/updates can validate quickly. For `cascade` / `set null` / `set default`, add an index on the referencing columns (child table). Without an index, Better-Convex will throw if it detects referencing rows.
</Callout>
...
You now know how to keep your data consistent with constraints and make queries fast with indexes. From unique fields to cascading deletes and vector search, Better-Convex gives you Drizzle's ergonomic API backed by Convex's runtime.

## Next Steps

<Cards>
  <Card title="Operators (Filters)" href="/docs/db/orm/operators" />
