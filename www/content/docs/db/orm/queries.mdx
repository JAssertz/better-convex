...
**Note**: Non-paginated `findMany()` requires explicit sizing: provide `limit`, use `paginate`, set `allowFullScan`, or configure `defineSchema(..., { defaults: { defaultLimit } })`.
</Callout>

### findFirst

Use `findFirst()` to retrieve a single row matching your criteria. This example uses `_id` which has a built-in index:
...
Use object filters (Drizzle v1 style) to narrow results. The shape mirrors Drizzle but is executed in Convex:
...
**Important**: When using predicate `where: (row) => ...`, you must provide an explicit `index` configuration. Without it, the query will fail rather than silently scanning every document.
</Callout>
...
Better-Convex supports familiar string matching operators. Most run post-fetch, but a few can leverage indexes:
...
Compound indexes follow Convex prefix rules. For an index on `[type, numLikes]`, `where: { numLikes: 10 }` is treated as non-indexable and requires `allowFullScan` unless you also constrain the leading field (`type`). Reversed `AND` equality order is normalized to index field order before query execution.
...
Drizzle exposes `exists(query)` and `notExists(query)` as SQL subquery operators. Better-Convex does not support SQL subqueries at runtime, so these operators are unavailable.
...
These operators are non-index-compiled and require `allowFullScan` unless paired with another indexable pre-filter. Here are scalable workarounds for each:
...
| predicate `where: (row) => ...` | Use `index: { name, range? }`; add `paginate.maxScan` to cap scan cost |
...
return db.query.posts.findMany({
  ...
  paginate: { cursor: args.cursor ?? null, limit: 20 },
});
```
...
| Complex JS predicate + pagination | `db.query.*.findMany({ where: (row) => ..., index: { name: 'by_field' }, paginate: { cursor, limit, maxScan } })` | Explicit index + bounded scan |
...
| Embedding similarity | `db.query.*.findMany({ vectorSearch: { index, vector, limit, includeScore?, filter? } })` | Similarity order from vector search; no `paginate`/`where`/`orderBy` |
...
<Cards>
  <Card title="Writing Data" href="/docs/db/orm/mutations">
