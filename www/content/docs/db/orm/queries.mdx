...
| predicate `where: (row) => ...` | Use `index: { name, range? }`; add `maxScan` (cursor pagination only) to cap scan cost |
...
| Complex JS predicate + pagination | `db.query.*.findMany({ where: (row) => ..., index: { name: 'by_field' }, cursor, limit, maxScan })` | Explicit index + bounded scan |
