...
Let's start with a mutation that renames a user by `_id`:
...
export const renameUser = publicMutation
  .input(z.object({ userId: zid('users'), name: z.string() }))
...
**Important:** `update()` without `.where(...)` throws unless you call `.allowFullScan()`. See [Querying Data](/docs/db/orm/queries) for details on `allowFullScan`.
</Callout>
...
For the full list of configurable defaults, see [Schema Definition -- Runtime Defaults](/docs/db/orm/schema#runtime-defaults).
...
<Callout icon={<AlertTriangle />}>
**Important:** Async execution requires wiring `ormFunctions` and `scheduledMutationBatch` in your ORM setup. See [Mutations -- Async Wiring Setup](/docs/db/orm/mutations#wiring-setup) for the setup steps.
</Callout>
