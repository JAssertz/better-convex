...
Configure default delete semantics per table with `deletion(...)`.
...
- Numeric defaults must be integers (`mutationAsyncDelayMs` allows `0`; others must be `>= 1`).

### Strict Policy (Guardrails)

Strict guardrails now cover relation indexes, relation fan-out cardinality, predicate where(), and unbounded update/delete.
