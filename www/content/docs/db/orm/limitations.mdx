---
title: Limitations
description: Understand ORM limitations, workarounds, and performance characteristics
---

import { AlertTriangle } from "lucide-react"

Better-Convex mirrors Drizzle’s relational query builder on top of Convex’s document database. That means a few SQL-only features do not apply, and some features are implemented post-fetch for API parity.
...
- Compound index planning follows prefix validity. Non-leading-only predicates (for example `numLikes` on `[type, numLikes]`) are treated as non-indexable and require `allowFullScan`
...
- Vector mode (`findMany({ vectorSearch })`) does not support `cursor`, `limit`, `maxScan`, `where`, `orderBy`, `index`, or `offset`
