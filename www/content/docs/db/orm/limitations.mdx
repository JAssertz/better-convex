...
Better-Convex ORM (M1-M4) implements core query operations. Some features are planned for future milestones, and some SQL-specific features don't apply to Convex's document model.

## Current Status (M1-M4)
...
| Category | Description | Examples (M1-M4) |
|----------|-------------|----------|
| **Category 1: Implemented** | Core features working now | findMany, findFirst, where, limit, offset |
...
This page documents **Category 2**, **Category 4**, and **planned features (M5+)**.

## Not Yet Implemented (M5+)

The following Drizzle-compatible features are planned but not yet available in M1-M4:

### Relation Loading with `with:`

```ts
// 🚧 NOT YET IMPLEMENTED - Coming in M5
...
```ts
// 🚧 NOT YET IMPLEMENTED - Coming in M5-M6
...
**Better-Convex (M1-M5):**
```ts
// ⚠️ Not available in M1-M5
const users = convexTable('users', {
  role: v.string(), // No .default()
  status: v.string(),
});

// ✅ WORKAROUND: Set in application code
const userId = await db(ctx).insert.users.values({
  name: 'Alice',
  email: 'alice@example.com',
...
**Better-Convex:**
```ts
// ⚠️ Not available in ORM layer
const users = convexTable('users', {
  email: v.string(), // No .unique()
});

// ✅ WORKAROUND: Use Convex Ents defineEnt()
...
**Better-Convex:**
❌ **Not available** - Convex uses document queries, not SQL
...
**Better-Convex:**
❌ **Not needed** - Convex is schemaless
...
**Better-Convex:**
❌ **Not needed** - Use edge-based relations instead
...
**Better-Convex:**
❌ **Not needed** - All mutations are automatically atomic
...
**Better-Convex:**
❌ **Not applicable** - Convex manages connections automatically
...
**✅ Automatic Prevention (Better-Convex):**
```ts
const users = await db(ctx).query.users.findMany({
  with: {
...
<Callout icon={<Zap />}>
**🚀 Performance Tip**: Use cursor pagination for any list over 1000 records or infinite scroll UI.
</Callout>
...
|-----------|---------------------|---------------|--------|
| **Find 10 users** | ~15ms | ~5ms | ✅ Convex (3x faster) |
| **Filter by role** | ~20ms | ~8ms | ✅ Convex (2.5x faster) |
| **Load user + posts** | ~30ms | ~12ms | ✅ Convex (2.5x faster) |
| **Cursor pagination (page 100)** | ~50ms | ~5ms | ✅ Convex (10x faster) |
| **Full-text search** | ~40ms | ~10ms | ✅ Convex (4x faster) |
