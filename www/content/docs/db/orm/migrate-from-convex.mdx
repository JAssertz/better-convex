...
For complex JavaScript predicates, Better-Convex supports predicate `where`, but it requires an explicit `index: { name, range? }` plan and should be bounded with `paginate.maxScan`.
    ...
    export const list = publicQuery
      ...
      .query(async ({ ctx, input }) => {
        return ctx.orm.query.posts.findMany({
          orderBy: { _creationTime: 'desc' },
          paginate: { cursor: input.cursor, limit: 20 },
        });
      });
    ```
  </CompareItem>
