...
ORM `findMany()` must be explicitly sized: use `limit`, `paginate`, set a schema `defaultLimit`, or opt in with `allowFullScan`.
</Callout>
...
For complex JavaScript predicates, Better-Convex supports predicate `where`, but it requires an explicit `index: { name, range? }` plan and should be bounded with `paginate.maximumRowsRead`.
...
<Compare>
  ...
  <CompareItem title="After (`findMany({ paginate })`)">
    ...
    export const list = publicQuery
      ...
      .query(async ({ ctx, input }) => {
        return ctx.orm.query.posts.findMany({
          orderBy: { _creationTime: 'desc' },
          paginate: { cursor: input.cursor, numItems: 20 },
        });
      });
    ```
  </CompareItem>
