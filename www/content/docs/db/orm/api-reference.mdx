...
## Stream & Pagination Helpers

Native fork of `convex-helpers` stream utilities, exported from `better-convex/orm`:

```ts
db.stream() // preferred entrypoint
db.stream(schema) // optional explicit schema
stream(ctx.db, schema)
mergedStream(streams, indexFields)
streamIndexRange(db, schema, table, index, bounds, order)
getPage(ctx, request)
paginator(db, schema)
```
