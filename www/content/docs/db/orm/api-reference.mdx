...
- `createdAt: Date` when `createOrm({ types: { date: true } })` and you call through cRPC
...
```ts
const orm = createOrm({ schema: relations, types: { date: true } });
const db = orm.db(ctx);
```
...
- `number` by default
- `Date` when `types: { date: true }` (recommended when using cRPC)
...
**Options**
- `where`: object filter (Drizzle shape) or predicate function
...
- `allowFullScan`: opt‑in for non-cursor object-filter full scans when no index is available (not supported with cursor pagination; not used with predicate `where`, `search`, or `vectorSearch`)
...
const page = await db.query.messages
  .select()
  .union([
    { index: { name: 'by_from_to', range: (q) => q.eq('from', me).eq('to', them) } },
    { index: { name: 'by_from_to', range: (q) => q.eq('from', them).eq('to', me) } },
  ])
...
- Predicate `where`: provide `index: { name, range? }` and use `maxScan` for bounded scans (cursor pagination only).
