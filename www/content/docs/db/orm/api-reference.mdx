...
## Mutation Operations (🚧 Coming in M5-M6)
...
## Ordering Functions (🚧 Coming in M5)

<Callout icon={<AlertTriangle />}>
**Not Yet Implemented**: `orderBy`, `asc`, and `desc` are planned for M5. Sort results manually after fetching for now.
</Callout>

The following APIs show the **planned design** for M5:

### orderBy() (Planned)

```ts
// NOT YET IMPLEMENTED
orderBy(field: Field | Field[]): OrderByConfig
```

**Workaround:** Sort after fetching:
```ts
const users = await db(ctx).query.users.findMany();
const sorted = users.sort((a, b) => b._creationTime - a._creationTime);
```
...
```ts
// NOT YET IMPLEMENTED
asc(field: Field): SortOrder
desc(field: Field): SortOrder
```
