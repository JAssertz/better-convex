...
- `limit`, `offset`
- `paginate`: `{ cursor, limit, maxScan? }` (cursor pagination)
...
Most queries should use `findMany()` / `findFirst()` (or `findFirstOrThrow()`).
...
- Predicate `where`: provide `index: { name, range? }` and use `paginate.maxScan` for bounded scans.
