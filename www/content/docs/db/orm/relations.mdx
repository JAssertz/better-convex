...
<Callout icon={<AlertTriangle />}>
Relation loading requires indexes on relation fields (e.g. `posts.authorId`). Missing indexes throw at runtime to avoid full scans. Set `defineSchema(..., { strict: false })` to allow unindexed scans with Convex warnings.
</Callout>
