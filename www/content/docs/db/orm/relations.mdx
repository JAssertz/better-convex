...
Relations are about **loading** related rows. They do **not** enforce referential integrity. If you want runtime checks / cascades, define foreign keys separately with `.references()` / `foreignKey()` (see [/docs/db/orm/indexes-constraints](/docs/db/orm/indexes-constraints)).
...
If you have **multiple relations between the same two tables**, you must use `alias` to tell Better-Convex which relation is which.
...
<Callout icon={<InfoIcon />}>
`where` in a relation definition can only reference **columns on the target (to) table**. In Better-Convex, it's applied after fetching candidate rows, so keep `many()` relations intentionally sized.
</Callout>

## Indexing For Relation Loading

Better-Convex will use indexes when loading relations and will throw (unless `allowFullScan`) if the needed index is missing.
...
- `one()`: `to: ..._id` uses `db.get()` (no extra index); non-`_id` targets require an index on the target fields
