...
Pagination and filtering are tightly coupled in Better-Convex.
...
|---|---|---|---|---|
| `findMany({ where: object })` | Optional (index-compiled when possible, otherwise `allowFullScan`) | `limit/offset`, `cursor + limit` | Yes (`where: { relation: ... }`) | `orderBy` supported; first field drives cursor stability |
| `findMany({ where: (row) => ..., index })` | Required explicit `index: { name, range? }` | `limit/offset`, `cursor + limit`, optional `maxScan` (cursor mode) | No (predicate replaces object relation filter path) | Index/stream-backed; first order field matters for cursor mode |
...
| `findMany({ pipeline })` | Schema + index/range path per union source (or inferred base path) | `cursor + limit` (+ optional `endCursor`, `maxScan`) or non-cursor | Manual via pipeline stages (`filterWith`, `map`, `flatMap`, `distinct`) | Stream-backed index order |
...
4. Need custom JS predicate logic? Use predicate `where` with explicit `index`.
5. Need advanced pre-pagination composition (union/interleave/map/filterWith/flatMap/distinct)? Use `findMany({ pipeline })`.

<Callout icon={<InfoIcon />}>
**Note:** The ORM supports search via `findMany({ search: { index, query, filters? } })`. Predicate `where` still runs post‑fetch and cannot be combined with search mode.
</Callout>

## Pipeline Filtering (Advanced)

Most filtering should stay in plain `findMany`. Use pipeline mode when you need stream-style transforms before pagination.

This example uses a union + interleave (internal merged-stream semantics), then applies `filterWith` and `map` before returning the page:

```ts title="convex/functions/messages.ts" showLineNumbers {13-32}
import { z } from 'zod';
...
export const listConversation = publicQuery
  .input(z.object({ me: zid('users'), them: zid('users') }))
  ...
  .query(async ({ ctx, input }) => {
    return await ctx.orm.query.messages.findMany({
      ...
      pipeline: {
        union: [
          { index: { name: 'by_from_to', range: (q) => q.eq('from', input.me).eq('to', input.them) } },
          { index: { name: 'by_from_to', range: (q) => q.eq('from', input.them).eq('to', input.me) } },
        ],
