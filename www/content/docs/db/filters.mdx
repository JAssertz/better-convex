...
<Callout icon={<InfoIcon />}>
**Important:** Always use `limit` or `paginate` with search queries to limit results.
</Callout>
...
export const searchPaginated = publicQuery
  ...
  .query(async ({ ctx, input }) => {
    return await ctx.orm.query.articles.findMany({
      ...
      paginate: { cursor: input.cursor, limit: input.limit },
    });
  });
```
...
| Pagination | Standard | Works with `paginate` |
...
- Always limit results with `limit` or `paginate`
...
const results = await db.query.posts.findMany({
  ...
  paginate: { cursor: input.cursor, limit: input.limit, maxScan: 500 },
});
```
...
- Prefer `findMany({ paginate })` for user‑facing lists
...
7. **Prefer cursor pagination** - Use `findMany({ paginate })` for large lists
