...
const likeCount = await aggregatePostLikes.count(ctx, {
  namespace: postId,
  bounds: {} as any, // Required for null keys
});
```
...
const count = await aggregate.count(ctx, { namespace: gameId, bounds: {} as any });
...
// Get random song
const count = await randomAggregate.count(ctx, { bounds: {} as any });
