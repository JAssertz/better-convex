...
```ts title="convex/lib/triggers.ts" showLineNumbers {1-6,10-11,13-16}
...
const song = result
  ? await ctx.orm.query.songs.findFirst({ where: { _id: result.doc._id } })
  : null;
```
...
export const backfillAggregate = privateMutation
  ...
  .mutation(async ({ ctx, input }) => {
    const results = await ctx.orm.query.scores.findMany({
      ...
      paginate: { limit: input.batchSize, cursor: input.cursor },
    });
...
triggers.register('user', async (ctx, change) => {
  if (change.operation === 'delete') {
    ...
    for (const char of characterRows) {
      ...
      await ctx.orm.delete(characters).where(eq(characters._id, char._id));
    }
  }
});
```
...
export const deleteScore = authMutation
  ...
  .mutation(async ({ ctx, input }) => {
    const doc = await ctx.orm.query.scores.findFirst({ where: { _id: input.scoreId } });
    ...
    await ctx.orm.delete(scores).where(eq(scores._id, input.scoreId));
  });
```
