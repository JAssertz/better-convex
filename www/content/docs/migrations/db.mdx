...
import { convexTable, defineRelations, defineSchema, text, boolean, integer, id, index } from 'better-convex/orm';
...
The migration is direct: `_id` equality through `where` is the ORM equivalent of a point lookup.
...
`findMany()` must be explicitly sized: provide `limit`, use cursor pagination (`cursor` + `limit`), set schema `defaultLimit`, or explicitly opt in with `allowFullScan`.
</Callout>
...
<Compare>
  ...
  <CompareItem title="Native Convex (manual join)">
    ...
    const authorById = new Map(authors.filter(Boolean).map((u) => [u._id, u]));
...
| Complex JS predicate filtering | `.filter(...)` (table/index scan semantics) | predicate `where` + explicit `index: { name, range? }` |
...
- Predicate `where` in ORM must include explicit `index` and should be bounded with `maxScan` in cursor mode.
