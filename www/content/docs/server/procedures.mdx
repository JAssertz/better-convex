...
export const getById = publicQuery
  ...
  .query(async ({ ctx, input }) => {
    return ctx.orm.query.user.findFirst({ where: { _id: input.id } });
  });
```
...
export const getById = publicQuery
  ...
  .query(async ({ ctx, input }) => {
    const user = await ctx.orm.query.user.findFirst({ where: { _id: input.id } });
...
export const remove = publicMutation
  .input(z.object({ id: zid('user') }))
  .mutation(async ({ ctx, input }) => {
    await ctx.orm.delete(user).where(eq(user._id, input.id));
  });
```
...
The handler receives flat `input.cursor` and `input.limit`. Pass `input` directly to `paginate`. The output is automatically typed as `{ continueCursor: string, isDone: boolean, page: T[] }`.
...
export const update = publicMutation
  ...
  .mutation(async ({ ctx, input }) => {
    const { id, ...data } = input;
    await ctx.orm.update(user).set(data).where(eq(user._id, id));
  });
```
  ...
  <CompareItem title="Before (vanilla Convex)">
    ...
    export const getById = query({
      ...
      handler: async (ctx, args) => {
        const db = orm.db(ctx);
        return db.query.user.findFirst({ where: { _id: args.id } });
      },
    });
    ```
  </CompareItem>
