...
```tsx title="app/posts/post-list.tsx"
'use client';
...
- Data that shouldn't be exposed to client
...
- Need server-side conditionals (404, redirects)
- Generate metadata from the data
- Render data directly in Server Components
...
1. **Prefer `prefetch`** - Let client components own the data
2. **Use `preloadQuery` only when needed** - For metadata, 404 checks, redirects
3. **Don't render fetched data in Server Components** if client components also use it
4. **If you must use both**, understand they can get out of sync after revalidation
...
Key differences:
- No prop drilling of preloaded data
- Client components use standard `useQuery` hooks
- `prefetch` is non-blocking (fire-and-forget)
- Same query options as client-side code
- Automatic hydration via `HydrateClient`
- Auth token configured once via `getToken` in QueryClient options

## Next Steps

<Cards>
  <Card title="Setup" href="/docs/nextjs/setup" />
