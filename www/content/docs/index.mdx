...
We didn't reinvent the wheel. Better Convex integrates battle-tested tools - Convex for realtime data, TanStack Query for caching, and Better Auth for authentication - behind a consistent API.
...
6. If `better-convex codegen` fails with `Local backend isn't running`, start `bunx better-convex dev` in another terminal and retry.
7. If browser smoke says use `http://localhost:3000` but port 3000 is occupied, run smoke on the actual dev server port instead and note it in the report.
...
| [Rate Limiting](/docs/server/advanced/rate-limiting) | Token bucket and fixed window rate limiting with `@convex-dev/rate-limiter`. |
...
| [Triggers](/docs/orm/triggers) | Automatic side effects on insert/update/delete with schema triggers. |
| [Aggregates](/docs/server/advanced/aggregates) | O(log n) counts and sums with `@convex-dev/aggregate`. Leaderboards, running totals. |
...
| [Auth Triggers](/docs/auth/triggers) | User triggers: `beforeCreate`, `onCreate`, `onDelete`. |
...
2. [Triggers](/docs/orm/triggers) → automatic side effects
3. [Aggregates](/docs/server/advanced/aggregates) → counts and sums
