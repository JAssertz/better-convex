...
Traditional Convex development requires assembling patterns yourself: auth checks in every function, manual cache management, custom context helpers, and separate packages for relationships.

Better Convex unifies these into a single, opinionated framework:
...
| Stack | Key Docs |
|-------|----------|
| Next.js + Better Auth + Ents | [Templates](/docs/templates), [Next.js Setup](/docs/nextjs/setup), [Auth Setup](/docs/auth/setup) |
...
| [Auth Setup](/docs/auth/setup) | Complete Better Auth setup: schema, adapters, HTTP routes, client. |
| [Auth Triggers](/docs/auth/triggers) | User lifecycle hooks: `beforeCreate`, `onCreate`, `onDelete`. |
| [Auth Helpers](/docs/auth/helpers) | `getAuthUserId`, `getSession`, `useAuthStatus`, `Authenticated`/`Unauthenticated` components. |
...
**Add authentication:**
1. [Auth Setup](/docs/auth/setup) → configure Better Auth
2. [Auth Helpers](/docs/auth/helpers) → use auth in components
